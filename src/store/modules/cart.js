...
export default {
  ...
  mutations: {
    // 加入购物车
    // goods包含：id,name,picture,price,nowPrice,count,skuId,attrsText,selected,stock,isEffective
    // 本地无需添加：isCollect discount
    insertCart (state, goods) {
      // 判断购物车是否又相同商品
      // 1. 如果有，把相同的商品的数据累加到新goods中，删除相同商品，将新的goods追加再前面
      // 2. 将新的goods追加再前面
      const sameIndex = state.list.findIndex(item => item.skuId === goods.skuId)
      if (sameIndex > -1) {
        const sameGoods = state.list[sameIndex]
        goods.count += sameGoods.count
    ...
    updateCartSku (ctx, { oldSkuId, newSku }) {
      return new Promise((resolve, reject) => {
        if (ctx.rootState.user.profile.token) {
          ...
          // 3. 合并一条新的商品信息
          const { skuId, price: nowPrice, stock, attrsText } = newSku
      ...
      return new Promise((resolve, reject) => {
        if (ctx.rootState.user.profile.token) {
          ...
          Promise.all(promiseArr).then(dataArr => {
            // dataArr 是所有成功结果集合，顺序和promiseArr顺序一致，和ctx.state.list顺序一致。
